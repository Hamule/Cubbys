...
void Player::SetBuildBlock(bool s)
{
	if (s)
	{
		m_equippedProperties = PlayerEquippedProperties::BuildBlock;
...
bool Player::IsBuild() const
{
	return(m_equippedProperties & PlayerEquippedProperties::BuildBlock) == PlayerEquippedProperties::BuildBlock;
}
...
void Player::RenderSelectionBlock() const
{
	...
	if ((IsBuild() == false&&IsPickaxe() == false) && IsBlockPlacing() == false && IsItemPlacing() == false)
	{
...
void Player::SetNowBlock(ItemType s)
{
	m_nowBlock = s;
	OutputDebugString(TEXT("Setnow"));
}
