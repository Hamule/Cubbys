...
void Player::PressAttack()
{
	...
	else if (IsPickaxe())
	{
		...
		if (CanAttackRight())
		{
			OutputDebugString(TEXT("Mineing"));
	...
	if (m_blockSelection)
	{
		...
		if (pChunk != nullptr)
		{
			...
			if (active)
			{
				...
				OutputDebugString(TEXT("Block Destroy"));
...
void Player::BuildBlock()
{
	if (m_blockSelection)
	{
		...
		if (pChunk != nullptr)
		{
			...
			if (active)
			{
				playerPos = m_position;
				BlockPos(playerPos, blockPos,x,y,z);
				pChunk->StartBatchUpdate();
				SetBlock(m_nowBlock,x,y,z);
				pChunk->SetUpdate();
				pChunk->StopBatchUpdate();
				OutputDebugString(TEXT("Block Build"));
			}
...
void Player::BlockPos(glm::vec3 ps, glm::vec3 bs, int &x, int &y, int &z)
{
	

	// 위아래 설치
	if ((sqrt(pow(ps.x - bs.x, 2) + pow(ps.z - bs.z, 2))) <1.5)
	{
		y = 1;
		if (ps.y <= bs.y -4)
		{
	...
	{
		// x 축 설치
		if (abs((sqrt(pow(ps.x - bs.x, 2) + pow(0 - 0, 2)))) > abs((sqrt(pow(0 - 0, 2) + pow(ps.z - bs.z, 2)))))
		{
			z = 0;
			if (ps.x > bs.x)
				x = 1;
		...
		// z 축 설치
		else if (abs((sqrt(pow(ps.x - bs.x, 2) + pow(0 - 0, 2)))) < abs((sqrt(pow(0 - 0, 2) + pow(ps.z - bs.z, 2)))))
		{
			x = 0;
			if (ps.z > bs.z)
				z = 1;
...
void Player::SetBlock(ItemType s, int x, int y, int z)
{
	OutputDebugString(TEXT("Block set"));
	if (s == ItemType::BlockDirt)
	{
		m_pChunkManager->ImportQubicleBinary("Resources/gamedata/terrain/blocks/Dirtblock.qb", glm::vec3(m_blockSelectionPos.x+x, m_blockSelectionPos.y + y, m_blockSelectionPos.z + 1 + z), QubicleImportDirection::Normal);
		
	}
	else if (s == ItemType::BlockSnow)
	{
		m_pChunkManager->ImportQubicleBinary("Resources/gamedata/terrain/blocks/Snowblock.qb", glm::vec3(m_blockSelectionPos.x + x, m_blockSelectionPos.y + y, m_blockSelectionPos.z + 1 + z), QubicleImportDirection::Normal);


	}
	else if (s == ItemType::BlockSand)
	{
		m_pChunkManager->ImportQubicleBinary("Resources/gamedata/terrain/blocks/Sandblock.qb", glm::vec3(m_blockSelectionPos.x + x, m_blockSelectionPos.y + y, m_blockSelectionPos.z + 1 + z), QubicleImportDirection::Normal);

	}
	else if (s == ItemType::BlockWood)
	{
		m_pChunkManager->ImportQubicleBinary("Resources/gamedata/terrain/blocks/Woodblock.qb", glm::vec3(m_blockSelectionPos.x + x, m_blockSelectionPos.y + y, m_blockSelectionPos.z + 1 + z), QubicleImportDirection::Normal);

	}
	else if (s == ItemType::BlockGrass)
	{
		m_pChunkManager->ImportQubicleBinary("Resources/gamedata/terrain/blocks/Grassblock.qb", glm::vec3(m_blockSelectionPos.x + x, m_blockSelectionPos.y + y, m_blockSelectionPos.z + 1 + z), QubicleImportDirection::Normal);
...
void Player::AttackAnimationTimerFinished()
{
	...
	else if (IsBuild())
	{
		...
		{
			OutputDebugString(TEXT("Building"));
